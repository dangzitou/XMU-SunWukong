	...
	export default {
		...
		methods: {
			...
			getRules() {
				...
				rules = rules ? rules[this.newprop||this.prop] : [];
				// 保证返回的是一个数组形式
				return [].concat(rules || []);
			},
			...
			validation(trigger, callback = () => {}) {
				...
				// 修改支持a.b
				this.fieldValue = this.getData(this.parent.model,this.newprop||this.prop);
				...
				let validator = new schema({
					[this.newprop||this.prop]: rules
				});
				validator.validate({
					[this.newprop||this.prop]: this.fieldValue
				}, {
					firstFields: true
				}, (errors, fields) => {
					// 记录状态和报错信息
			...
			resetField() {
				...
				this.setData(this.parent.model,this.newprop||this.prop,this.initialValue);
		...
		beforeDestroy() {
			...
			if (this.parent && (this.newprop||this.prop)) {
